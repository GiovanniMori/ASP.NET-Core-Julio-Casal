...

{
	...
	public static RouteGroupBuilder MapGamesEndpoints(this WebApplication app)
	{
		...
		games.FirstOrDefault(g => g.Id == id) is GameDto game ? Results.Ok(game) : Results.NotFound()).WithName(GetGameEndpointName);

		group.MapPost("/games", (CreateGameDto gameDto) =>
		{
			var game = new GameDto(games.Max(g => g.Id) + 1, gameDto.Name,
				gameDto.Genre,
				gameDto.Price,
				gameDto.ReleaseDate);
			games.Add(game);
